...
public class GeneticPathFinder : MonoBehaviour {
	...
	private void Update() {
		if (hasBeenInitialized && !hasFinished) {
			// creature finishes if it's reached the end of its path
			if (pathIndex == dna.genes.Count || Vector2.Distance(transform.position, target) < 0.5f) {
				hasFinished = true;
			}

			// creature's path. indexes everytime the creature's position equals it's next point.
